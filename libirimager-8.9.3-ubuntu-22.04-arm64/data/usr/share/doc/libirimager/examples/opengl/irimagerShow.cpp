...
FramerateCounter fpsThermalFrames;

double _elapsed            = 0.0;
int    _cntElapsed         = 0;
int    _cntFrames          = 0;
int    _radiusMinMax       = 1;       // Controls radius of measurementfield of hotspot and coldspot detection
...
void onThermalFrame(unsigned short* thermal, unsigned int w, unsigned int h, IRFrameMetadata meta, void* arg)
{
  ...
  fpsThermalFrames.trigger();
  _tempChip = meta.tempChip;
  _tempFlag = meta.tempFlag;
  _tempBox = meta.tempBox;
...
void* displayWorker(void* arg)
{
  ...
  while(viewer.isAlive() && !_shutdown)
  {
    ...
    if(drawCrosshair)
    {
      ...
      _iBuilder.getMinMaxRegion(_radiusMinMax, &minRegion, &maxRegion);
...
int main(int argc, char* argv[])
{
  ...
  for (unsigned int i = 0; i < optics->size(); i++)
  {
    ...
    std::cout << "FOV: " << op.fov << " deg, Text: " << op.text.data() << std::endl;
  ...
  int measurementfieldX, measurementfieldY;
  measurementfieldX = measurementfieldY = _radiusMinMax * 2 + 1;  // +1 for center pixle

  std::cout << "Size of Hotspot and Coldspot measurement field: " << measurementfieldX << "x" << measurementfieldY << std::endl << std::endl; 
  ...
  if(_imager->init(&_params, dev->getFrequency(), dev->getWidth(), dev->getHeight(), dev->controlledViaHID(), dev->getHwRev(), dev->getFwRev()))
  {
    ...
    cout << "Thermal channel: " << _imager->getWidth() << "x" << _imager->getHeight() << endl;

    _biSpectral = _imager->hasBispectralTechnology();
    if(_biSpectral)
      cout << "Visible channel: " << _imager->getVisibleWidth() << "x" << _imager->getVisibleHeight() << endl;
    // -------------------------------------------------------
      ...
      case IRIMAGER_STATE_ACQUIRE:
        ...
        if(retval==IRIMAGER_SUCCESS)
        {
          ...
          if(fpsStream.trigger())
          {
            cout << "------" <<                                                                   "\n" <<
                    "--Frame" <<                                                                  "\n" <<
                    "Framerate (in/from device): " << fpsStream.getFps() << " fps " <<            "\n" <<
                    "Framerate (out/thermal Frames): " << fpsThermalFrames.getFps() << " fps" <<  "\n" <<
                    "Elapsed (process call): " << _elapsed/(double)_cntElapsed << " ms" <<        "\n" <<
                    "Frames: " << _cntFrames <<                                                   "\n" <<
                    "Dropped: " << cntDropped <<                                                  "\n" <<
                    "--Temperatures" <<                                                           "\n" <<
                    "Chip Temp: " << _tempChip <<                                                 "\n" <<
                    "Flag Temp: " << _tempFlag <<                                                 "\n" <<
                    "Box Temp: " << _tempBox << endl;
            _elapsed    = 0.0;
            _cntElapsed = 0;
          }
          usleep(1000);
